...
Voice.on("ready", async() => {
    ...
    Channel.join().then(connection =>{
        ...
        Voice.log("Ses Dosyası Çalınıyor...")
        if(!Channel.hasStaff()) playVoice(Voice);
        else Voice.staffJoined = true;

    }).catch(err => {
        Voice.error(`Ses Kanalına Bağlanılamadı (${Channel.name}) (${Channel.id}): ` + err.message)
        return Voice.destroy();
    });
...
Voice.login(CONFIG.TOKEN).catch(err => {
    Voice.error("Ses Kanalına Bağlanırken Sorun Oluştu: " + err.message);
    return Voice.destroy();
});
...
function playVoice(Voice) {
    try {
        ...
        return Voice.log("Ses Dosyalarını Çalarken Sorun Oluştu: " + err.message);
